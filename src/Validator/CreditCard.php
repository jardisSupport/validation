...
final class CreditCard implements ValueValidatorInterface
{
    ...
    /**
     * Static helper: Diners Club validation.
     *
     * @return array<string, mixed>
     */
    public static function diners(): array
    {
        return ['cardType' => 'diners'];
    }

    /**
     * Static helper: JCB validation.
     *
     * @return array<string, mixed>
     */
    public static function jcb(): array
    {
        return ['cardType' => 'jcb'];
    }
    ...
    public function validateValue(mixed $value, array $options = []): ?string
    {
        ...
        if ($cardType !== null) {
            ...
            if (!isset(self::CARD_PATTERNS[$cardTypeLower])) {
                return $hasCustomMessage ? $message : sprintf('Unknown card type: %s', $cardType);
            }

            if (!preg_match(self::CARD_PATTERNS[$cardTypeLower], $cardNumber)) {
                return $hasCustomMessage ? $message : sprintf('Invalid %s card number', ucfirst($cardTypeLower));
            }
        }
