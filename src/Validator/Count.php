...
final class Count implements ValueValidatorInterface
{
    ...
    public function validateValue(mixed $value, array $options = []): ?string
    {
        ...
        // Exact count validation — exclusive of min/max
        if ($exact !== null) {
            ...
            if ($count !== $exact) {
                return $hasCustomMessage
                    ? $message
                    : sprintf('Must contain exactly %d element(s), got %d', $exact, $count);
            }
            return null;
        }

        // Min validation (optional kombiniert mit max)
        if ($min !== null) {
            if ($count < $min) {
                return $hasCustomMessage
                    ? $message
                    : sprintf('Must contain at least %d element(s), got %d', $min, $count);
            }
            if ($max !== null && $count > $max) {
                return $hasCustomMessage
                    ? $message
                    : sprintf('Must contain at most %d element(s), got %d', $max, $count);
            }
            return null;
        }
