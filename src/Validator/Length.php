...
final class Length implements ValueValidatorInterface
{
    ...
    public function validateValue(mixed $value, array $options = []): ?string
    {
        ...
        if (isset($options['exact'])) {
            ...
            if ($length !== $exact) {
                return $hasCustomMessage
                    ? $message
                    : sprintf('String must be exactly %d %s, got %d', $exact, $unit, $length);
            }
            return null;
        }
        ...
        if (isset($options['min']) && $length < $options['min']) {
            ...
            return $hasCustomMessage
                ? $message
                : sprintf('String too short: minimum %d %s required, got %d', $min, $unit, $length);
        }
        ...
        if (isset($options['max']) && $length > $options['max']) {
            ...
            return $hasCustomMessage
                ? $message
                : sprintf('String too long: maximum %d %s allowed, got %d', $max, $unit, $length);
        }
