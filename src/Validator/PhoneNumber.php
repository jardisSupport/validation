    ...
    public function validateValue(mixed $value, array $options = []): ?string
    {
        ...
        if ($countryCode !== null) {
            ...
            if (!isset(self::COUNTRY_PATTERNS[$countryCodeUpper])) {
                return $hasCustomMessage
                    ? $message
                    : sprintf('Unsupported country code: %s', $countryCode);
            }

            if (!preg_match(self::COUNTRY_PATTERNS[$countryCodeUpper], $cleaned)) {
                return $hasCustomMessage
                    ? $message
                    : sprintf('Invalid phone number for country %s', $countryCodeUpper);
            }
        ...
        if ($requireInternational) {
            if (!preg_match('/^\+[1-9]\d{1,14}$/', $cleaned)) {
                return $hasCustomMessage
                    ? $message
                    : 'Phone number must be in international format (+XX...)';
            }
            return null;
        }
