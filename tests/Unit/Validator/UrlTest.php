...
final class UrlTest extends TestCase
{
    ...
    {
        $custom = 'Custom URL error for all cases';

        // Protocol mismatch
        $result = $this->validator->validateValue(
            'http://example.com',
            ['message' => $custom, 'allowedProtocols' => ['https']]
        );
        ...
        $result = $this->validator->validateValue(
            'http://localhost',
            ['message' => $custom, 'allowLocalhost' => false]
        );
        $this->assertSame($custom, $result);

        // Basic invalid URL
        $result = $this->validator->validateValue('not-a-url', ['message' => $custom]);
        $this->assertSame($custom, $result);
    }
