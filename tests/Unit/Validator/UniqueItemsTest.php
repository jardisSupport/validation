...
final class UniqueItemsTest extends TestCase
{
    ...
    public function testLooseDuplicateReportingNoDuplicateEntries(): void
    {
        // [1, '1', 1]: In loose mode, 1 and '1' are duplicates.
        // The third value 1 should not cause a second duplicate entry.
        $result = $this->validator->validateValue([1, '1', 1], ['strict' => false]);
        $this->assertIsString($result);

        // Count occurrences of the duplicate value in the error message
        // Should only list the duplicate once, not twice
        $this->assertSame(1, substr_count($result, "'1'"), 'Duplicate should be listed only once');
    }

    public function testCustomMessageIsRespectedForAllErrors(): void
    {
        $custom = 'Custom unique error for all cases';

        // Strict duplicates
        $result = $this->validator->validateValue([1, 2, 1], ['message' => $custom, 'strict' => true]);
        $this->assertSame($custom, $result);

        // Loose duplicates
        $result = $this->validator->validateValue([1, '1'], ['message' => $custom, 'strict' => false]);
        $this->assertSame($custom, $result);
    }
