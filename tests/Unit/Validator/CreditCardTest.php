...
final class CreditCardTest extends TestCase
{
    ...
    public function testDinersHelper(): void
    {
        $options = CreditCard::diners();
        $this->assertIsArray($options);
        $this->assertArrayHasKey('cardType', $options);
        $this->assertSame('diners', $options['cardType']);
    }

    public function testJcbHelper(): void
    {
        $options = CreditCard::jcb();
        $this->assertIsArray($options);
        $this->assertArrayHasKey('cardType', $options);
        $this->assertSame('jcb', $options['cardType']);
    }

    public function testValidDinersCards(): void
    {
        $dinersCards = [
            '30569309025904',
            '38520000023237',
        ...
        foreach ($dinersCards as $card) {
            $result = $this->validator->validateValue($card, ['cardType' => 'diners']);
            $this->assertNull($result, "Expected '{$card}' to be valid Diners");
        }
    }

    public function testValidJcbCards(): void
    {
        $jcbCards = [
            '3530111333300000',
            '3566002020360505',
        ...
        foreach ($jcbCards as $card) {
            $result = $this->validator->validateValue($card, ['cardType' => 'jcb']);
            $this->assertNull($result, "Expected '{$card}' to be valid JCB");
        }
    }

    public function testNonDigitWithCustomMessage(): void
    {
        $customMessage = 'Custom error';
        $result = $this->validator->validateValue('4532-0151-1283-036X', ['message' => $customMessage]);
        $this->assertSame($customMessage, $result);
    }
    ...
    {
        $custom = 'Custom card error for all cases';

        // Unknown card type
        $result = $this->validator->validateValue(
            '4532015112830366',
            ['message' => $custom, 'cardType' => 'unknown']
        );
        $this->assertSame($custom, $result);

        // Invalid card type pattern (Visa number as Mastercard)
        $result = $this->validator->validateValue(
            '4532015112830366',
            ['message' => $custom, 'cardType' => 'mastercard']
        );
        $this->assertSame($custom, $result);
