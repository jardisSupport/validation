...
final class CountTest extends TestCase
{
    ...
    {
        $custom = 'Custom count error for all cases';

        // Exact mismatch
        $result = $this->validator->validateValue([1, 2], ['message' => $custom, 'exact' => 3]);
        $this->assertSame($custom, $result);

        // Too few
        $result = $this->validator->validateValue([1], ['message' => $custom, 'min' => 3]);
        $this->assertSame($custom, $result);

        // Too many
        $result = $this->validator->validateValue([1, 2, 3, 4, 5, 6], ['message' => $custom, 'max' => 5]);
        $this->assertSame($custom, $result);
    }
