...
final class PositiveTest extends TestCase
{
    ...
    {
        $custom = 'Custom positive error for all cases';

        // Non-numeric value
        $result = $this->validator->validateValue('not-a-number', ['message' => $custom]);
        $this->assertSame($custom, $result);

        // Negative number
        $result = $this->validator->validateValue(-1, ['message' => $custom]);
        $this->assertSame($custom, $result);

        // Zero without allowZero
        $result = $this->validator->validateValue(0, ['message' => $custom]);
        $this->assertSame($custom, $result);

        // Negative with allowZero
        $result = $this->validator->validateValue(-1, ['message' => $custom, 'allowZero' => true]);
        $this->assertSame($custom, $result);
    }
