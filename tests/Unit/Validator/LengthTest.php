...
final class LengthTest extends TestCase
{
    ...
    {
        $custom = 'Custom length error for all cases';

        // Exact mismatch
        $result = $this->validator->validateValue('1234', ['message' => $custom, 'exact' => 5]);
        $this->assertSame($custom, $result);

        // Too short
        $result = $this->validator->validateValue('hi', ['message' => $custom, 'min' => 5]);
        $this->assertSame($custom, $result);

        // Too long
        $result = $this->validator->validateValue('verylongstring', ['message' => $custom, 'max' => 10]);
        $this->assertSame($custom, $result);
    }
