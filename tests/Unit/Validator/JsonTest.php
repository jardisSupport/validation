...
final class JsonTest extends TestCase
{
    ...
    public function testPrimitiveWithExpectedTypeObject(): void
    {
        // Primitives should not be classified as 'object'
        $primitives = ['null', '123', 'true', '"string"'];

        foreach ($primitives as $primitive) {
            $result = $this->validator->validateValue($primitive, ['expectedType' => 'object']);
            $this->assertIsString($result, "Expected primitive '{$primitive}' to fail object type check");
            $this->assertStringContainsString('primitive', $result);
        }
    }

    public function testPrimitiveWithExpectedTypeArray(): void
    {
        $result = $this->validator->validateValue('123', ['expectedType' => 'array']);
        $this->assertIsString($result);
        $this->assertStringContainsString('primitive', $result);
    }
...
JSON;
    ...
    {
        $custom = 'Custom JSON error for all cases';

        // Invalid JSON syntax
        $result = $this->validator->validateValue('{invalid}', ['message' => $custom]);
        $this->assertSame($custom, $result);

        // Primitive when expecting object
        $result = $this->validator->validateValue(
            '123',
            ['message' => $custom, 'expectedType' => 'object']
        );
        $this->assertSame($custom, $result);
        ...
        // Object when expecting array
        $result = $this->validator->validateValue(
            '{"key":"value"}',
            ['message' => $custom, 'expectedType' => 'array']
        );
        $this->assertSame($custom, $result);

        // Empty string
        $result = $this->validator->validateValue('', ['message' => $custom]);
        $this->assertSame($custom, $result);
    }
