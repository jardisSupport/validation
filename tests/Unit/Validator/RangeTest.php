...
final class RangeTest extends TestCase
{
    ...
    {
        // 'Ä' is 1 character but 2 bytes — mb_strlen must be used
        $result = $this->validator->validateValue('Ä', ['min' => 1, 'max' => 1]);
        $this->assertNull($result);

        // 'Ää' is 2 characters
        $result = $this->validator->validateValue('Ää', ['min' => 1, 'max' => 1]);
        $this->assertIsString($result);
        $this->assertStringContainsString('too long', strtolower($result));

        // '日本語' is 3 characters
        $result = $this->validator->validateValue('日本語', ['min' => 3, 'max' => 3]);
        $this->assertNull($result);
    }

    public function testCustomMessageIsRespectedForAllErrors(): void
    {
        $custom = 'Custom range error for all cases';

        // Numeric too small
        $result = $this->validator->validateValue(0, ['message' => $custom, 'min' => 1, 'max' => 10]);
        $this->assertSame($custom, $result);

        // Numeric too big
        $result = $this->validator->validateValue(11, ['message' => $custom, 'min' => 1, 'max' => 10]);
        $this->assertSame($custom, $result);

        // String too short
        $result = $this->validator->validateValue('ab', ['message' => $custom, 'min' => 3, 'max' => 10]);
        $this->assertSame($custom, $result);

        // String too long
        $result = $this->validator->validateValue('this is a very long string', ['message' => $custom, 'min' => 3, 'max' => 10]);
        $this->assertSame($custom, $result);
    }

    public function testStringWithFloatBoundaries(): void
    {
        // Float boundaries should work for string length validation
        $result = $this->validator->validateValue('hello', ['min' => 3.5, 'max' => 10.5]);
        $this->assertNull($result);

        $result = $this->validator->validateValue('hi', ['min' => 3.5, 'max' => 10.5]);
        $this->assertIsString($result);
    }
}
