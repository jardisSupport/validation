...
final class PhoneNumberTest extends TestCase
{
    ...
    {
        $custom = 'Custom phone error for all cases';

        // Unsupported country code
        $result = $this->validator->validateValue(
            '+12025551234',
            ['message' => $custom, 'countryCode' => 'XX']
        );
        ...
        $result = $this->validator->validateValue(
            '12025551234',
            ['message' => $custom, 'requireInternational' => true]
        );
        $this->assertSame($custom, $result);

        // Basic invalid
        $result = $this->validator->validateValue('abc', ['message' => $custom]);
        $this->assertSame($custom, $result);
    }
