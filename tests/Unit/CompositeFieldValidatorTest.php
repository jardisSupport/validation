...
final class CompositeFieldValidatorTest extends TestCase
{
    ...
    public function testPsrGetterResolution(): void
    {
        $validator = new CompositeFieldValidator();
        $validator
            ->field('email')
    ...
    public function testIsBooleanGetterResolution(): void
    {
        $validator = new CompositeFieldValidator();
        $validator
            ->field('active')
    ...
    public function testHasBooleanGetterResolution(): void
    {
        $validator = new CompositeFieldValidator();
        $validator
            ->field('permission')
    ...
    public function testGetterPriorityOverReflection(): void
    {
        $validator = new CompositeFieldValidator();
        $validator
            ->field('email')
        ...
        // Getter returns valid email, property holds invalid value
        $data = new class {
            protected string $email = 'not-an-email';
    ...
    public function testGetterPriorityOverUcfirstConvention(): void
    {
        $validator = new CompositeFieldValidator();
        $validator
            ->field('email')
        ...
        // get*() has priority over ucfirst convention
        $data = new class {
            public function getEmail(): string
            {
    ...
    public function testUcfirstFallbackStillWorks(): void
    {
        $validator = new CompositeFieldValidator();
        $validator
            ->field('email')
        ...
        // Only ucfirst convention available — should still work
        $data = new class {
            public function Email(): string
            {
                return 'user@example.com';
    ...
    public function testValidatesFollowedByBreaksOnRegistersAll(): void
    {
        $validator = new CompositeFieldValidator();
        $validator
            ->field('name')
                ->validates(NotBlank::class)
                ->breaksOn(Length::class, Length::min(3))
            ->field('email')
                ->validates(Email::class);

        // name = 'ab' → NotBlank passes (not blank), Length breaks (too short), email not validated
        $data = new class {
        ...
        // breaksOn(Length) should break, so no errors are collected
        $this->assertTrue($result->isValid());

        // Now with valid name: break doesn't trigger, email gets validated
        $data2 = new class {
            public function name(): string
            {
                return 'abc';
        ...
        $this->assertArrayHasKey('email', $result2->getErrors());
    }

    public function testValidatesPendingFlushedBeforeBreaksOn(): void
    {
        // Verifies that validates() configs are flushed when breaksOn() is called
        // so that validates(NotBlank) is registered as NORMAL before breaksOn(Length) is registered as BREAK
        $validator = new CompositeFieldValidator();
        $builder = $validator
            ->field('name')
                ->validates(NotBlank::class);

        // At this point NotBlank is still in the buffer
        $configs = $builder->getValidatorConfigs();
        $this->assertCount(1, $configs);

        // After breaksOn, finalize should have been called, clearing the buffer
        $builder->breaksOn(Length::class, Length::min(3));
        $configs = $builder->getValidatorConfigs();
        $this->assertCount(0, $configs);
    }

    public function testBreakValidatorRespectsExcludeFields(): void
    {
        $validator = new CompositeFieldValidator();
        $validator
            ->field('name')
        ...
        // Object without id → excluded fields are skipped (create mode)
        // Break validator on 'name' should be skipped, email should be validated
        $data = new class {
            public ?string $id = null;
        ...
        // name is excluded (id=null), so breaksOn(NotBlank) should NOT trigger
        // email should be validated and fail
        $this->assertFalse($result->isValid());
        $this->assertArrayHasKey('email', $result->getErrors());
    }

    public function testWithIdentityFieldUsesCustomIdentityForSkipDecision(): void
    {
        $validator = new CompositeFieldValidator();
        $validator
            ->field('email')
                ->validates(Email::class)
            ->excludeFields(['email'])
            ->withIdentityField('uuid');

        // Object without uuid → create mode → excluded fields are skipped
        $data = new class {
        ...
        // Object with uuid set → update mode → excluded fields are validated
        $data2 = new class {
